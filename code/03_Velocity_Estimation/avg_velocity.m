...
if (~isempty(loc))&&(~isempty(vf))
    for i = 1:min(size(loc,1),size(vf,1))
        if (loc(i,1)>0)&& (loc(i,1)<= img_sz(1))&&(loc(i,2)>0)&&(loc(i,2)<= img_sz(2))
            var_x(ceil(loc(i,1)),ceil(loc(i,2))) = abs(vf(i,1) - ux_avg(ceil(loc(i,1)),ceil(loc(i,2))));
            var_y(ceil(loc(i,1)),ceil(loc(i,2))) = abs(vf(i,2) - uy_avg(ceil(loc(i,1)),ceil(loc(i,2))));
        end
    end
end
Var_x = imfilter(double(var_x),gaussian1,'replicate'); 
Var_y = imfilter(double(var_y),gaussian1,'replicate');
var_vel = sqrt(Var_x.^2 + Var_y.^2);
