...
function [trainingImages,trainingLabel]=ExtractFeatures_subFold(TrainingSet_Path,para)
Folder_list =  dir([TrainingSet_Path]);
...
for folder_idx = 1:NumFolder
        if(para.W==1)
            Img_list = dir([TrainingSet_Path '\' Folder_list(folder_idx+2).name '\' para.ImgType ]) ;
             if(size(Img_list,1)==0)
                Img_list = dir([TrainingSet_Path '\' Folder_list(folder_idx+2).name '\*.BMP' ]);
            end
        else
            Img_list = dir([TrainingSet_Path '/' Folder_list(folder_idx+2).name '/*.bmp']) ; %  para.ImgType 
            if(size(Img_list,1)==0)
                Img_list = dir([TrainingSet_Path '/' Folder_list(folder_idx+2).name '/*.BMP' ]);
            end
        end
    ...
    for idx = 1:NumImg(folder_idx) 
        %--------------------------------
        if(para.W==1)
            img = imread([TrainingSet_Path '\' Folder_list(folder_idx+2).name '\' Img_list(idx).name  ]);   
        else
            img = imread([TrainingSet_Path '/' Folder_list(folder_idx+2).name '/' Img_list(idx).name  ]);
        end
        if (folder_idx==1)
