...
%   pFlow.type       ='LK'; % - ['LK'] may be 'LK', 'HS' or 'SD'
%   pFlow.smooth     = 1;   % -    [1] smoothing radius for triangle filter (may be 0)
%   pFlow.filt       = 0;   % -    [0] median filtering radius for smoothing flow field
%   pFlow.minScale   = 1/4% - [1/64] minimum pyramid scale (must be a power of 2)
%   pFlow.maxScale   = 1/2% -    [1] maximum pyramid scale (must be a power of 2)
%   pFlow.radius     % -   [10] integration radius for weighted window [LK/SD only]
%   pFlow.nBlock     % -    [5] number of tested blocks [SD only]
%   pFlow.alpha      % -    [1] smoothness constraint [HS only]
%   pFlow.nIter      % -  [250] number of iterations [HS only]
%
% OUTPUTS
